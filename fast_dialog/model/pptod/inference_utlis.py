...
def batch_generate(model, one_inference_batch, ref_bs, ref_act, ref_db, input_contain_db, data, cls_name, args):
    '''
        e2e evaluation: 
            ref_bs: False
            ref_act: False
    ...
    if evaluation_setup == 'e2e':
        ...
        bs_tensor, bs_mask = cls_name.pad_batch(bs_batch)
        if is_cuda:
        ...
        da_tensor, da_mask = cls_name.pad_batch(da_batch_input_id_list)
        if is_cuda:
        ...
        nlg_tensor, nlg_mask = cls_name.pad_batch(nlg_batch_input_id_list)
        if is_cuda:
        ...
        da_tensor, da_mask = cls_name.pad_batch(da_batch)
        if is_cuda:
            da_tensor = da_tensor.cuda(device)
            da_mask = da_mask.cuda(device)
        batch_da_text = model.batch_generate(da_tensor, da_mask, generate_mode='da', max_decode_len=max_span_len)
        ...
        nlg_tensor, nlg_mask = cls_name.pad_batch(nlg_batch)
        if is_cuda:
            nlg_tensor = nlg_tensor.cuda(device)
            nlg_mask = nlg_mask.cuda(device)
        batch_nlg_text = model.batch_generate(nlg_tensor, nlg_mask, generate_mode='nlg', max_decode_len=max_response_len)
