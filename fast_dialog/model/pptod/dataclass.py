...
def collate_fn(batch):
    def parse_batch(raw_batch):
        ...
        for item in raw_batch:
            ...
            input_data_list.extend(one_input_data_list)

            one_output_data_list = []
            for key in ['bs_output', 'da_output', 'nlg_output']:
                one_output_data_list.append(item[key])
            output_data_list.extend(one_output_data_list)
        return [input_data_list, output_data_list]

    batch = parse_batch(batch)

    return MultiWozData.parse_batch_tensor(batch)

def collate_fn_eval(batch):
    def parse_batch(raw_batch):
        bs_input_batch_list, da_input_batch_list, nlg_input_batch_list, parse_dict_batch_list = [], [], [], []
        for bs, da, nlg, parse_dict in raw_batch:
            bs_input_batch_list.append(bs)
            da_input_batch_list.append(da)
            nlg_input_batch_list.append(nlg)
            parse_dict_batch_list.append(parse_dict)
        
        return [bs_input_batch_list, da_input_batch_list, nlg_input_batch_list, parse_dict_batch_list]
    
    batch = parse_batch(batch)

    return batch

class MultiWozDatasetForDataLoader(Dataset):
    '''
    For Distributed Sampler and DataLoader.
    '''
...
class MultiWozEvalDatasetForDataLoader(Dataset):
    '''
    Eval Dataset for Distributed Sampler and DataLoader.
    '''
    def __init__(self, bs_input_id_list, da_input_id_list, nlg_input_id_list, parse_dict_list):
        super().__init__()
        self.bs_input_id_list = bs_input_id_list
        self.da_input_id_list = da_input_id_list
        self.nlg_input_id_list = nlg_input_id_list
        self.parse_dict_list = parse_dict_list

        assert len(bs_input_id_list) != 0 and len(da_input_id_list) != 0 \
            and len(nlg_input_id_list) != 0 and len(parse_dict_list) != 0

    def __len__(self):
        return len(self.bs_input_id_list)

    def __getitem__(self, idx):
        return self.bs_input_id_list[idx], self.da_input_id_list[idx], \
            self.nlg_input_id_list[idx], self.parse_dict_list[idx]
...
class MultiWozData:
    ...
    def __init__(self, model_name, tokenizer, cfg, data_path_prefix, shuffle_mode='shuffle_session_level', 
        ...
        if data_mode == 'train':
            ...
            train_data_id_list = self.tokenize_raw_data(train_raw_data, 'train')
        ...
        dev_data_id_list = self.tokenize_raw_data(dev_raw_data, 'dev')
        ...
        test_data_id_list = self.tokenize_raw_data(test_raw_data, 'test')
        ...
        self.train_dataset_for_dataloader = MultiWozDatasetForDataLoader(self.train_data_list)
    ...
    def tokenize_raw_data(self, raw_data_list, mode):
        '''
        Adding Cache
        '''
        all_session_list = []
        ...
        cached_file_path = os.path.join(self.cfg.fine_processed_path, mode + 'cached.json')
        if os.path.exists(cached_file_path):
            with open(cached_file_path, 'r') as f:
                all_session_list = json.loads(f.read())
            
            assert len(all_session_list) == len(raw_data_list)
            print("Loading cached from cached file %s", cached_file_path)
            return all_session_list

        print("Starting toeknizing now")
    ...
    @classmethod
    def pad_batch(cls, batch_id_list):
        batch_id_list = [torch.LongTensor(item) for item in batch_id_list]
        batch_tensor = rnn.pad_sequence(batch_id_list, batch_first=True, padding_value=cls.pad_token_id)
        batch_mask = torch.ones_like(batch_tensor)
        batch_mask = batch_mask.masked_fill(batch_tensor.eq(cls.pad_token_id), 0.0).type(torch.FloatTensor)
        return batch_tensor, batch_mask

    @classmethod
    def process_output(cls, batch_tgt_id_list):
        batch_tgt_id_list = [torch.LongTensor(item) for item in batch_tgt_id_list]
        batch_tgt_tensor, _ = cls.pad_batch(batch_tgt_id_list) # padded target sequence
    ...
    @classmethod
    def parse_batch_tensor(cls, batch):
        batch_input_id_list, batch_output_id_list = batch
        batch_src_tensor, batch_src_mask = cls.pad_batch(batch_input_id_list)
        batch_input, batch_labels = cls.process_output(batch_output_id_list)
        batch_labels[batch_labels[:, :] == cls.pad_token_id] = -100
    ...
    def build_all_evaluation_batch_list(self, ref_bs, ref_act, ref_db, input_contain_db, eva_batch_size, eva_mode, for_dataloader=False):
        '''
        ...
        if for_dataloader:
            return MultiWozEvalDatasetForDataLoader(all_bs_input_id_list, all_da_input_id_list, all_nlg_input_id_list, all_parse_dict_list)
