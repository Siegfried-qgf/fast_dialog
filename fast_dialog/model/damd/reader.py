...
from config.config_damd import Config
...
class _ReaderBase(object):
    ...
    def _construct_mini_batch(self, data):
        ...
        if (len(batch)%len(self.config.cuda_device)) != 0:
            batch = batch[:-(len(batch)%len(self.config.cuda_device))]
        if len(batch) > 0.5 * self.config.batch_size:
            all_batches.append(batch)
    ...
    def save_result(self, write_mode, results, field, write_title=False):
        with open(self.config.result_path, write_mode) as rf:
            if write_title:
    ...
    def save_result_report(self, results):
        ...
        #     with open(self.config.result_path[:-4] + '_report_dst.txt', 'w') as rf:
        ...
        #     with open(self.config.result_path[:-4] + '_report_bsdx.txt', 'w') as rf:
        #         rf.write('joint goal\tslot_acc\tslot_f1\tact_f1\n')
        #         for res in results:
        #             a,b,c,d = res['joint_goal_delex'], res['slot_acc_delex'], res['slot_f1_delex'], res['act_f1']
        #             rf.write('%2.1f\t%2.1f\t%2.1f\t%2.1f\n'%(a,b,c,d))
        ctr_save_path = self.config.result_path[:-4] + '_report_ctr%s.csv'%self.config.seed
        write_title = False if os.path.exists(ctr_save_path) else True
        if self.config.aspn_decode_mode == 'greedy':
            setting = ''
        elif self.config.aspn_decode_mode == 'beam':
            setting = 'width=%s'%str(self.config.beam_width)
            if self.config.beam_diverse_param>0:
                setting += ', penalty=%s'%str(self.config.beam_diverse_param)
        elif self.config.aspn_decode_mode == 'topk_sampling':
            setting = 'topk=%s'%str(self.config.topk_num)
        elif self.config.aspn_decode_mode == 'nucleur_sampling':
            setting = 'p=%s'%str(self.config.nucleur_p)
        res = {'exp': self.config.eval_load_path, 'true_bspn':self.config.use_true_curr_bspn, 'true_aspn': self.config.use_true_curr_aspn,
                  'decode': self.config.aspn_decode_mode, 'param':setting, 'nbest': self.config.nbest, 'selection_sheme': self.config.act_selection_scheme,
...
class MultiWozReader(_ReaderBase):
    def __init__(self, config):
        ...
        self.db = MultiWozDB(self.config.dbs) #TODO meiodng
        self.vocab_size = self._build_vocab()
        self.domain_files = json.loads(open(self.config.domain_file_path, 'r').read())
        self.slot_value_set = json.loads(open(self.config.slot_value_set_path, 'r').read())
        if self.config.multi_acts_training:
            self.multi_acts = json.loads(open(self.config.multi_acts_path, 'r').read())

        test_list = [l.strip().lower() for l in open(self.config.test_list, 'r').readlines()]
        dev_list = [l.strip().lower() for l in open(self.config.dev_list, 'r').readlines()]
        ...
        self.exp_files = {}
        if 'all' not in self.config.exp_domains:
            for domain in self.config.exp_domains:
        ...
        if self.config.limit_bspn_vocab:
            self.bspn_masks = self._construct_bspn_constraint()
        if self.config.limit_aspn_vocab:
            self.aspn_masks = self._construct_aspn_constraint()

        self.multi_acts_record = None

    def _build_vocab(self):
        self.vocab = utils.Vocab(self.config.vocab_size)
        vp = self.config.vocab_path_train if self.config.mode == 'train' or self.config.vocab_path_eval is None else self.config.vocab_path_eval
        # vp = self.config.vocab_path+'.json.freq.json'
    ...
    def _load_data(self, save_temp=False):
        self.data = json.loads(open(self.config.data_path+self.config.data_file, 'r', encoding='utf-8').read().lower())
        self.train, self.dev, self.test = [] , [], []
        for fn, dial in self.data.items():
            if 'all' in self.config.exp_domains or self.exp_files.get(fn):
    ...
    def convert_batch(self, py_batch, py_prev, first_turn=False):
        ...
        if first_turn:
            ...
            for item, py_list in py_prev.items():
                if py_list is None:
                    continue
                if not self.config.enable_aspn and 'aspn' in item:
                    continue
                if not self.config.enable_bspn and 'bspn' in item:
                    continue
                if not self.config.enable_dspn and 'dspn' in item:
                    continue
                prev_np = utils.padSeqs(py_list, truncated=self.config.truncated, trunc_method='pre')
        ...
        for item in ['user', 'usdx', 'resp', 'bspn', 'aspn', 'bsdx', 'dspn']:
            if not self.config.enable_aspn and item == 'aspn':
                continue
            if not self.config.enable_bspn and item == 'bspn':
                continue

            if not self.config.enable_dspn and item == 'dspn':
                continue
            py_list = py_batch[item]
            trunc_method = 'post' if item == 'resp' else 'pre'
            # max_length = self.config.max_nl_length if item in ['user', 'usdx', 'resp'] else self.config.max_span_length
            inputs[item+'_np'] = utils.padSeqs(py_list, truncated=self.config.truncated, trunc_method=trunc_method)
        ...
        if self.config.multi_acts_training and self.config.mode=='train':
            ...
            for bidx, aspn_type_list in enumerate(py_batch['aspn_aug']):
                if aspn_type_list:
                    for aspn_list in aspn_type_list:
                        ...
                        if self.config.multi_act_sampling_num>1:
                            for i in range(self.config.multi_act_sampling_num):
            ...
            if multi_aspn:
                inputs['aspn_aug_np'] = utils.padSeqs(multi_aspn, truncated=self.config.truncated, trunc_method='pre')
    ...
    def record_utterance(self, result_dict):
        ...
        for dial_id, turns in result_dict.items():
            ...
            for turn_no, turn in enumerate(turns):
                ...
                act_type_collect = {}
                slot_score = 0
                for i in range(self.config.nbest):
        ...
        with open(self.config.eval_load_path + '/dialogue_record.csv', 'w') as rf:
            ...
            # json.dump(dialog_record, open(self.configeval_load_path + '/resultdict.json','w'))
