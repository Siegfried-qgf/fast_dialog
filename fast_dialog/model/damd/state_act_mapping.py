...
def dialog_turn_state_analysis(mode, config):
    data_path = os.path.join(config.data_path, config.data_file)
    conv_data = config.raw_data_path

    convlab_data = json.loads(open(conv_data, 'r').read().lower())
    reader = MultiWozReader(config)
    ...
    with open(os.path.join(config.data_path, 'multi_act_mapping_%s.json'%mode), 'w') as f:
...
if __name__ == '__main__':
    ...
    dialog_turn_state_analysis('train', config)